...
int find_cpus_allowed(struct tp_env *env)
{
	 ...
	 * - mask2 : cpus_binding_mask
	 */
	cpumask_copy(&mask[0], cpu_active_mask);
	cpumask_copy(&mask[1], &env->p->cpus_allowed);
	cpumask_copy(&mask[2], cpus_binding_mask(env->p));
	...
	if (env->per_cpu_kthread) {
		cpumask_copy(&env->cpus_allowed, &mask[1]);
		goto out;
	}

	/*
	 * Given task must run on the CPU combined as follows:
	 *	cpu_active_mask & task_cpu_possible_mask
	 */
...
out:
	if (test_ti_thread_flag(&env->p->thread_info, TIF_32BIT)) {
		/*
		 * cpus_allowed of 32bits task contains only the cpu that supports
		 * 32bits. If cpus_allowed does not intersect p->cpus_allowed, the
		 * cpu allowed by cpu sparing among p->cpus_allowed is selected. If
		 * all cpus of p->cpus_allowed are not allowed by cpu sparing,
		 * select first cpu of 32bit el0 supported cpus.
		 */
		if (!cpumask_intersects(&env->cpus_allowed, &mask[1])) {
			struct cpumask temp;

			cpumask_and(&temp, &mask[0], &mask[1]);
