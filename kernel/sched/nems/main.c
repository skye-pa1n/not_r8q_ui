...
{
	pr_info("\t\tcomm=%s pid=%d prio=%d isa=%s",
		p->comm, p->pid, p->prio,
...
ems_fops_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case EMS_IOCTL_R_ECS_DOMAIN_COUNT:
	{
		...
		if (!access_ok(user_count, NULL, size)) {
			ret = -EFAULT;
			break;
		}

		if (copy_to_user(user_count, NULL, size)) {
	...
	case EMS_IOCTL_R_ECS_DOMAIN_INFO:
	{
		...
		if (!access_ok(user_info, NULL, size)) {
			ret = -EFAULT;
			break;
		}

		if (copy_to_user(user_info, NULL, size)) {
	...
	case EMS_IOCTL_W_ECS_TARGET_DOMAIN_ID:
	{
		...
		if (!access_ok(user_target_domain_id, NULL, size)) {
			ret = -EFAULT;
			break;
		}
	...
	case EMS_IOCTL_W_ECS_TARGET_STAGE_ID:
	{
		...
		if (!access_ok(user_target_stage_id, NULL, size)) {
			ret = -EFAULT;
			break;
		}
	...
	case EMS_IOCTL_W_ECS_STAGE_THRESHOLD:
	{
		...
		if (!access_ok(user_threshold, NULL, size)) {
			ret = -EFAULT;
			break;
		}
