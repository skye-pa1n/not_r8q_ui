...
struct uaggov_policy {
	struct cpufreq_policy	*policy;
	u64			last_ws;
	u64			curr_cycles;
	u64			last_cyc_update_time;
	unsigned long		avg_cap;
	struct uaggov_tunables	*tunables;
...
struct uaggov_cpu {
#if LINUX_VERSION_CODE > KERNEL_VERSION(5, 10, 0)
	struct uaggov_callback	cb;
#else
	struct update_util_data	update_util;
#endif
	struct uaggov_policy	*wg_policy;
	unsigned int		cpu;
...
static DEFINE_PER_CPU(struct uaggov_cpu, uaggov_cpu);
static unsigned int stale_ns;
static DEFINE_PER_CPU(struct uaggov_tunables *, cached_tunables);
...
static bool uaggov_should_update_freq(struct uaggov_policy *wg_policy, u64 time)
{
	s64 delta_ns;
...
static bool uaggov_up_down_rate_limit(struct uaggov_policy *wg_policy, u64 time,
				     unsigned int next_freq)
{
	s64 delta_ns;
...
static bool uaggov_update_next_freq(struct uaggov_policy *wg_policy, u64 time,
					unsigned int next_freq,
					unsigned int raw_freq)
{
	...
	if (uaggov_up_down_rate_limit(wg_policy, time, next_freq)) {
...
static unsigned long freq_to_util(struct uaggov_policy *wg_policy,
				  unsigned int freq)
{
...
#define KHZ 1000
static void uaggov_track_cycles(struct uaggov_policy *wg_policy,
...
static void uaggov_calc_avg_cap(struct uaggov_policy *wg_policy, u64 curr_ws,
				unsigned int prev_freq)
{
	...
	if (curr_ws > (last_ws + sched_ravg_window)) {
		...
		uaggov_track_cycles(wg_policy, prev_freq, curr_ws);
...
static void uaggov_fast_switch(struct uaggov_policy *wg_policy, u64 time,
			      unsigned int next_freq)
{
	struct cpufreq_policy *policy = wg_policy->policy;

	uaggov_track_cycles(wg_policy, wg_policy->policy->cur, time);
	cpufreq_driver_fast_switch(policy, next_freq);
}

static void uaggov_deferred_update(struct uaggov_policy *wg_policy, u64 time,
				  unsigned int next_freq)
{
	if (use_pelt())
 ...
 */
static unsigned int get_next_freq(struct uaggov_policy *wg_policy,
				  unsigned long util, unsigned long max,
				  struct uaggov_cpu *wg_cpu, u64 time)
{
	struct cpufreq_policy *policy = wg_policy->policy;
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(5, 10, 0)
	trace_uaggov_next_freq(policy->cpu, util, max, raw_freq, freq, policy->min, policy->max,
	...
	if (!uaggov_update_next_freq(wg_policy, time, final_freq, freq))
		return 0;
...
#ifdef CONFIG_SCHED_WALT
static unsigned long uaggov_get_util(struct uaggov_cpu *wg_cpu)
{
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)
	return uag_cpu_util(wg_cpu->cpu, util, max,
				  FREQUENCY_UTIL, NULL);
#else
	return uclamp_rq_util_with(rq, util, NULL);
#endif
...
#endif
static int find_target_boost(unsigned long util, struct uaggov_policy *wg_policy,
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)
#ifdef CONFIG_NO_HZ_COMMON
static bool uaggov_cpu_is_busy(struct uaggov_cpu *wg_cpu)
{
#if LINUX_VERSION_CODE > KERNEL_VERSION(4, 14, 0)
...
#else
static inline bool uaggov_cpu_is_busy(struct uaggov_cpu *wg_cpu) { return false; }
...
#ifdef CONFIG_SCHED_WALT
static void uaggov_walt_adjust(struct uaggov_cpu *wg_cpu, unsigned long cpu_util,
				unsigned long nl, unsigned long *util,
				unsigned long *max)
{
	struct uaggov_policy *wg_policy = wg_cpu->wg_policy;
#if LINUX_VERSION_CODE > KERNEL_VERSION(5, 10, 0)
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 19, 0)
	if (unlikely(!sysctl_sched_use_uag_cpu_util))
		return;
#else
	if (is_rtg_boost)
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(4, 19, 0)
/*
 * Make uaggov_should_update_freq() ignore the rate limit when DL
 * has increased the utilization.
 */
static inline void ignore_dl_rate_limit(struct uaggov_cpu *wg_cpu, struct uaggov_policy *wg_policy)
{
...
static inline unsigned long target_util(struct uaggov_policy *wg_policy,
				  unsigned int freq)
{
	unsigned long util;
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)
static void uaggov_update_single(struct update_util_data *hook, u64 time,
				unsigned int flags)
{
	struct uaggov_cpu *wg_cpu = container_of(hook, struct uaggov_cpu, update_util);
	struct uaggov_policy *wg_policy = wg_cpu->wg_policy;
	struct cpufreq_policy *policy = wg_policy->policy;
	...
	if (!uaggov_should_update_freq(wg_policy, time))
		return;

	/* Limits may have changed, don't skip frequency update */
	busy = use_pelt() && !wg_policy->need_freq_update &&
		uaggov_cpu_is_busy(wg_cpu);

	wg_cpu->util = util = uaggov_get_util(wg_cpu);
	max = wg_cpu->max;
	wg_cpu->flags = flags;
	...
	uaggov_calc_avg_cap(wg_policy, wg_cpu->walt_load.ws,
	...
	for_each_cpu(j, policy->cpus) {
		struct uaggov_cpu *j_wg_cpu = &per_cpu(uaggov_cpu, j);
		...
		uaggov_walt_adjust(wg_cpu, j_util, j_nl, &util, &max);
	}
#endif
	...
	if (wg_policy->policy->fast_switch_enabled) {
		uaggov_fast_switch(wg_policy, time, next_f);
	} else {
		raw_spin_lock(&wg_policy->update_lock);
		uaggov_deferred_update(wg_policy, time, next_f);
		raw_spin_unlock(&wg_policy->update_lock);
	}
}
#endif

static unsigned int uaggov_next_freq_shared(struct uaggov_cpu *wg_cpu, u64 time)
{
	struct uaggov_policy *wg_policy = wg_cpu->wg_policy;
	struct cpufreq_policy *policy = wg_policy->policy;
...
#ifdef CONFIG_SCHED_WALT
		uaggov_walt_adjust(j_wg_cpu, j_util, j_nl, &util, &max);
#endif
	}
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(5, 10, 0)
static void uaggov_update_freq(struct uaggov_callback *cb, u64 time,
				unsigned int flags)
{
	struct uaggov_cpu *wg_cpu = container_of(cb, struct uaggov_cpu, cb);
#else
static void uaggov_update_freq(struct update_util_data *hook, u64 time,
				unsigned int flags)
{
	struct uaggov_cpu *wg_cpu = container_of(hook, struct uaggov_cpu, update_util);
#endif
	...
	wg_cpu->util = uaggov_get_util(wg_cpu);
	wg_cpu->flags = flags;
	raw_spin_lock(&wg_policy->update_lock);
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(5, 10, 0)
	trace_uaggov_util_update(wg_cpu->cpu, wg_cpu->util, wg_policy->avg_cap,
#else
...
#endif
		next_f = uaggov_next_freq_shared(wg_cpu, time);
...
static void uaggov_work(struct kthread_work *work)
{
	struct uaggov_policy *wg_policy = container_of(work, struct uaggov_policy, work);
	unsigned int freq;
	unsigned long flags;
	...
	uaggov_track_cycles(wg_policy, wg_policy->policy->cur,
			   ktime_get_ns());
...
static void uaggov_irq_work(struct irq_work *irq_work)
{
	struct uaggov_policy *wg_policy;

	wg_policy = container_of(irq_work, struct uaggov_policy, irq_work);
...
static inline struct uaggov_tunables *to_uaggov_tunables(struct gov_attr_set *attr_set)
{
	return container_of(attr_set, struct uaggov_tunables, attr_set);
}

static DEFINE_MUTEX(min_rate_lock);

static void update_min_rate_limit_ns(struct uaggov_policy *wg_policy)
{
...
{
	struct uaggov_tunables *tunables = to_uaggov_tunables(attr_set);
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(5, 10, 0)
		uaggov_run_callback(rq, WALT_CPUFREQ_BOOST_UPDATE);
#else
		hs_util = target_util(wg_policy,
					wg_policy->tunables->hispeed_freq);
		wg_policy->hispeed_util = hs_util;
...
uaggov_ATTR_RW(adaptive_low_freq);
uaggov_ATTR_RW(adaptive_high_freq);
uaggov_ATTR_RW(target_load_thresh);
uaggov_ATTR_RW(target_load_shift);

static struct attribute *uaggov_attributes[] = {
...
static struct kobj_type uaggov_tunables_ktype = {
	.default_attrs	= uaggov_attributes,
	.sysfs_ops	= &governor_sysfs_ops,
};

/********************** cpufreq governor interface *********************/

static struct cpufreq_governor uag_gov;

static struct uaggov_policy *uaggov_policy_alloc(struct cpufreq_policy *policy)
{
	struct uaggov_policy *wg_policy;
...
static void uaggov_policy_free(struct uaggov_policy *wg_policy)
{
	kfree(wg_policy);
}

static int uaggov_kthread_create(struct uaggov_policy *wg_policy)
{
	struct task_struct *thread;
	...
	init_irq_work(&wg_policy->irq_work, uaggov_irq_work);
...
static void uaggov_kthread_stop(struct uaggov_policy *wg_policy)
{
	/* kthread only required for slow path */
	if (wg_policy->policy->fast_switch_enabled)
		return;
...
static void uaggov_tunables_save(struct cpufreq_policy *policy,
		struct uaggov_tunables *tunables)
{
	int cpu;
	struct uaggov_tunables *cached = per_cpu(cached_tunables, policy->cpu);
...
static void uaggov_tunables_restore(struct cpufreq_policy *policy)
{
	struct uaggov_policy *wg_policy = policy->governor_data;
	struct uaggov_tunables *tunables = wg_policy->tunables;
	struct uaggov_tunables *cached = per_cpu(cached_tunables, policy->cpu);
...
static int uaggov_init(struct cpufreq_policy *policy)
{
	struct uaggov_policy *wg_policy;
	struct uaggov_tunables *tunables;
	int ret = 0;
	...
	wg_policy = uaggov_policy_alloc(policy);
	if (!wg_policy) {
		ret = -ENOMEM;
		goto disable_fast_switch;
	}

	ret = uaggov_kthread_create(wg_policy);
	if (ret)
		goto free_wg_policy;
	...
	uaggov_tunables_restore(policy);

	ret = kobject_init_and_add(&tunables->attr_set.kobj, &uaggov_tunables_ktype,
...
fail:
	...
	kfree(tunables);
stop_kthread:
	uaggov_kthread_stop(wg_policy);
free_wg_policy:
	uaggov_policy_free(wg_policy);
disable_fast_switch:
...
static void uaggov_exit(struct cpufreq_policy *policy)
{
	struct uaggov_policy *wg_policy = policy->governor_data;
	struct uaggov_tunables *tunables = wg_policy->tunables;
	unsigned int count;
	...
	policy->governor_data = NULL;
	if (!count) {
		uaggov_tunables_save(policy, tunables);
		kfree(tunables);
	}
...
static int uaggov_start(struct cpufreq_policy *policy)
{
	struct uaggov_policy *wg_policy = policy->governor_data;
	unsigned int cpu;
	...
	for_each_cpu(cpu, policy->cpus) {
		struct uaggov_cpu *wg_cpu = &per_cpu(uaggov_cpu, cpu);
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(5, 10, 0)
		uaggov_add_callback(cpu, &wg_cpu->cb, uaggov_update_freq);
#else
		cpufreq_add_update_util_hook(cpu, &wg_cpu->update_util,
					     policy_is_shared(policy) ?
							uaggov_update_freq :
							uaggov_update_single);
#endif
	}
...
static void uaggov_stop(struct cpufreq_policy *policy)
{
	struct uaggov_policy *wg_policy = policy->governor_data;
	unsigned int cpu;
...
static void uaggov_limits(struct cpufreq_policy *policy)
{
	struct uaggov_policy *wg_policy = policy->governor_data;
	...
	if (!policy->fast_switch_enabled) {
		...
		if (uaggov_update_next_freq(wg_policy, now, final_freq,
			final_freq)) {
			uaggov_fast_switch(wg_policy, now, final_freq);
		}
		raw_spin_unlock_irqrestore(&wg_policy->update_lock, flags);
	}
...
static struct cpufreq_governor uag_gov = {
	...
	.start			= uaggov_start,
	.stop			= uaggov_stop,
	.limits			= uaggov_limits,
};

static int __init uaggov_register(void)
{
	return cpufreq_register_governor(&uag_gov);
}
fs_initcall(uaggov_register);
