 ...
 * @sg_policy: muqss_sched policy object to compute the new frequency for.
 ...
 */
unsigned long muqss_sched_cpu_util(int cpu, unsigned long util_cfs,
				 ...
				 struct task_struct *p)
{
	...
	if (sched_feat(SUGOV_RT_MAX_FREQ) && !IS_BUILTIN(CONFIG_UCLAMP_TASK) &&
	    type == FREQUENCY_UTIL && if (rt_rq_runnable(rq)) {
		return max;
	}
...
static unsigned long sugov_get_util(struct sugov_cpu *sg_cpu)
{
	...
	return muqss_sched_cpu_util(sg_cpu->cpu, util_cfs, max,
				  FREQUENCY_UTIL, NULL);
}
#endif
...
unsigned long sched_cpu_util(int cpu)
{
	...
	return muqss_sched_cpu_util(cpu, cpu_util_cfs(cpu_rq(cpu)), max, ENERGY_UTIL, NULL);
}
...
static struct cpufreq_governor muqss_sched_gov;
...
static int sugov_init(struct cpufreq_policy *policy)
{
	...
	if (cpumask_test_cpu(policy->cpu, cpu_lp_mask)) {
		...
		tunables->down_rate_limit_us = 3142;
	}

	if (cpumask_test_cpu(policy->cpu, cpu_perf_mask)) {
		tunables->up_rate_limit_us = 450;
		tunables->down_rate_limit_us = 2138;
	}

        if (cpumask_test_cpu(policy->cpu, cpu_prime_mask)) {
                tunables->up_rate_limit_us = 500;
                tunables->down_rate_limit_us = 1872;
        }
...
{
		return &muqss_sched_gov;
}
#endif

static int __init sugov_register(void)
{
	return cpufreq_register_governor(&muqss_sched_gov);
}
