...
#define PM_QOS_CLUSTER0_FREQ_MIN 691200
#define PM_QOS_CLUSTER1_FREQ_MIN 710400
#define PM_QOS_CLUSTER2_FREQ_MIN 844000
...
static DEFINE_PER_CPU(struct sugov_snap, sugov_snap);
...
static struct cpufreq_governor nobody_sched_gov;
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_NOBODY_SCHED
struct cpufreq_governor *cpufreq_default_governor(void)
{
	return &nobody_sched_gov;
}
#endif
static void sugov_update_min(struct cpufreq_policy *policy)
{
	int cpu, max_cap;
	struct sugov_snap *sg_snap;
	int min_cap;
	...
	for_each_cpu(cpu, policy->cpus) {
		sg_snap = &per_cpu(sugov_snap, cpu);
		sg_snap->min = min_cap;
	}
}
...
static void sugov_start_slack(int cpu)
{
	struct sugov_snap *sg_snap = &per_cpu(sugov_snap, cpu);

	if (!sg_snap->enabled)
		return;

	sg_snap->min = ULONG_MAX;
	sg_snap->started = true;
}
...
static void sugov_stop_slack(int cpu)
{
	...
	sg_snap->started = false;
	if (timer_pending(&sg_snap->timer))
		del_timer_sync(&sg_snap->timer);
}
...
static int sugov_need_slack_timer(unsigned int cpu)
{
	...
	if (sg_cpu->util > sg_snap->min &&
		get_next_event_time_ms(cpu) > sg_snap->expired_time)
		return 1;
						...
						unsigned long action, void *v)
{
	...
	struct sugov_snap *sg_snap = &per_cpu(sugov_snap, cpu);
	struct sugov_cpu *sg_cpu = &per_cpu(sugov_cpu, cpu);
	struct timer_list *timer = &sg_snap->timer;
	...
	case CPU_PM_ENTER_PREPARE:
		...
		if (sugov_need_slack_timer(cpu)) {
			timer->expires = jiffies + msecs_to_jiffies(sg_snap->expired_time);
			add_timer_on(timer, cpu);
			trace_sugov_slack(cpu, sg_cpu->util, sg_snap->min, action, 1);
		}
		break;
...
static int find_cpu_pm_qos_class(int pm_qos_class)
{
	...
	for_each_possible_cpu(cpu) {
		...
		if ((sg_snap->qos_min_class == pm_qos_class) &&
...
static int __init sugov_parse_dt(struct device_node *dn, int cpu)
{
	...
	/* parsing slack info */
	if (of_property_read_u32(dn, "enabled", &sg_snap->enabled))
		return -EINVAL;
	if (sg_snap->enabled)
		if (of_property_read_u32(dn, "expired_time", &sg_snap->expired_time))
			sg_snap->expired_time = DEFAULT_EXPIRED_TIME;

	/* parsing pm_qos_class info */
	if (of_property_read_u32(dn, "qos_min_class", &sg_snap->qos_min_class))
		return -EINVAL;

	return 0;
}

static void __init sugov_snap_init(void)
{
	int cpu, ret;
	...
	for_each_possible_cpu(cpu) {
		...
		timer_setup(&sg_snap->timer, NULL, 0);

		sg_snap->timer.function = sugov_nop_timer;
	}
...
static int __init sugov_register(void)
{
	...
	return cpufreq_register_governor(&nobody_sched_gov);
}
